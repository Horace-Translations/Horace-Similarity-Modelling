...
# Extract texts, authors, and dates
...
# Labels: include index, author, year, and first few words
...
# --- Optional: Most common 3-word phrases ---
vectorizer_3gram = CountVectorizer(ngram_range=(4, 4)).fit(texts_clean)
...
# --- Similarity network graph (filtered and sized) ---
G = nx.Graph()

# Set top 0.5% threshold
top_percent = 0.005
...
# Get max and min weights for color normalization
...
# --- Draw graph with node size based on degree ---
...
# Draw edges with color intensity based on similarity
for i, j in G.edges():
...
# Convert string dates to integers and round down to 50-year periods
...
# Assign each translation to a 50-year group
half_centuries = [get_half_century(d) for d in dates]

# Group indices by 50-year periods
...
# Compute average similarity score per group
